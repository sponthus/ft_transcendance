...
function createFrienDiv(friend: any): HTMLElement {
	...
	const srcImg = `https://${window.location.hostname}:4443/uploads/${friend.avatar}`;
	
	append(userIcon, [(createImage(`user-notification-${friend.slug}`, 'w-[90%] aspect-square rounded-full object-cover object-center', `${srcImg}?t=${Date.now()}`) as HTMLImageElement)]);
