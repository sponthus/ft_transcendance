...
function createSlidingSearchBar() : HTMLElement {
	const slidingSearchBar = createDiv('sliding-search-bar', 'absolute right-0 top-0 w-0 overflow-hidden transition-all duration-300 ease-in-out bg-white rounded-full shadow-lg')
...
function openSearch() {
	isSearchOpen = true;
	(document.getElementById("sliding-search-bar-div") as HTMLElement).className = 'absolute right-0 top-0 w-72 overflow-hidden transition-all duration-300 ease-in-out bg-white rounded-full shadow-lg border-2 border-emerald-300';
...
function closeSearch() {
	...
	setTimeout(() => {
		(document.getElementById("sliding-search-bar-div") as HTMLElement).className = 'absolute right-0 top-0 w-0 overflow-hidden transition-all duration-300 ease-in-out bg-white rounded-full shadow-lg';
		searchInput.value = '';
	}, 150);
}
